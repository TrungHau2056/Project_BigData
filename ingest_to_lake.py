...
#         df = df.filter((col("price") >= 0) | col("price").isNull())
    ...
    df = df.withColumn(
        "event_date",
        F.to_date(col("event_time"))
    )

    df = (
        df
        .withColumn("category_parts", F.split(col("category_code"), r"\."))
        .withColumn("category_level_1", col("category_parts")[0])
        .withColumn("category_level_2", col("category_parts")[1])
        .withColumn("category_level_3", col("category_parts")[2])
        .drop("category_parts")
    )
